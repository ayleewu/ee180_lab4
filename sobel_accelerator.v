...
// Added regs and wires for the Sobel Algorithm 
reg [11:0] x_sum_top [`NUM_SOBEL_ACCELERATORS-1:0];
reg [11:0] x_sum_bottom [`NUM_SOBEL_ACCELERATORS-1:0];
reg [11:0] y_sum_left [`NUM_SOBEL_ACCELERATORS-1:0];
reg [11:0] y_sum_right [`NUM_SOBEL_ACCELERATORS-1:0];
...
generate
    ...
    for (c = 0; c < `NUM_SOBEL_ACCELERATORS; c = c + 1) begin: convolve
        always @(*) begin
            ...
            // Extracts all necessary 6 horizontal components for the output
            ...
            x_sum_top[c] = convx11[c] + convx12[c] + convx13[c];
            x_sum_bottom[c] = convx31[c] + convx32[c] + convx33[c];

            if (x_sum_top[c] >= x_sum_bottom[c])
                convx[c] = x_sum_top[c] - x_sum_bottom[c];
            else
                convx[c] = x_sum_bottom[c] - x_sum_top[c];
            ...
            // Combine the values above in a way that faithfully implements Sobel.
            // You may declare more signals as needed.
            y_sum_left[c] = convy11[c] + convy21[c] + convy31[c];
            y_sum_right[c] = convy13[c] + convy23[c] + convy33[c];

            if (y_sum_left[c] >= y_sum_right[c])
                 convy[c] = y_sum_left[c] - y_sum_right[c];
            else
                 convy[c] = y_sum_right[c] - y_sum_left[c];
            ...
            if (sobel_sum[c] > 12'd255)
                    sobel_out[(c+1)*8-1:(c)*8] = 8'hFF;
            else
